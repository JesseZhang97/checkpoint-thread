...
完整 V2.2 使用 Codex plugin 安装。它会同时启用 skill、`PreToolUse` guard 和
`PostToolUse` settle：
...
只需要指令层、接受没有自动 attribution 时，仍可通过 skills.sh 安装：
...
触发它。Hook 只观察明确的写操作，忽略普通 build/test 和 Git delivery 命令；
记录失败时警告并放行，不阻断正常工作。
